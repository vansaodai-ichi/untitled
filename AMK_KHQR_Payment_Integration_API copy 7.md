...
| `data.paymentId` | BIGINT | Unique payment identifier |
| `data.policyId` | VARCHAR(20) | Policy or application number |
| `data.customerName` | VARCHAR(100) | Customer's full name |
| `data.amount` | DECIMAL(18,2) | The total amount the customer is required to pay for the transaction, specified in the selected currency. |
| `data.currency` | VARCHAR(3) | Currency code (USD, KHR) |
| `data.qrCode` | TEXT | Base64 encoded QR code image |
| `timestamp` | TIMESTAMP | Response timestamp in ISO 8601 format |
| `correlationId` | VARCHAR(50) | Request correlation ID for tracking (generated by client or server) |
