...
Confirm a traditional bill payment transaction.

**Endpoint:** `POST /gateway/payments`
...
| `policyNumber` | string | Yes | Policy number (9-10 characters) |
| `amount` | number | Yes | Payment amount (positive value) |
| `currency` | string | Yes | Currency code (USD, KHR) |
| `bankTransactionId` | string | Yes | Bank transaction identifier |
| `payer` | string | Yes | Customer name |
| `paidDate` | string | Yes | Payment date (yyyyMMddHHmmss format) |

#### Example Request
```bash
curl --location 'https://{dai-ichi_base_url}/gateway/payments' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer eyJhbGciOiJIUzI1NiIs...' \
--header 'X-Request-ID: payment-confirm-67890' \
--data '{
    "id": 150001,
    "policyNumber": "P123456789",
    "amount": 150.00,
    "currency": "USD",
    "bankTransactionId": "BANK_TXN_150001",
    "payer": "John Doe",
...
```json
{
  ...
  "timestamp": "2025-10-07T12:00:00Z",
  "correlationId": "payment-confirm-67890"
}
```
